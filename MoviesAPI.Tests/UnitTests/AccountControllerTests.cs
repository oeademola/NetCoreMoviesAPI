...
namespace MoviesAPI.Tests.UnitTests
{
    ...
    public class AccountControllerTests: BaseTests
    {
        ...
        [TestMethod]
        public async Task UserLoginNotSuccessful()
        {
            //Preparation
            var databaseName = Guid.NewGuid().ToString();
            await CreateUser(databaseName);
            var userInfo = new UserInfo() {EmailAddress = "oluwaseyi@gmail.com", Password = "BadPassword"};

            //Testing
            var controller = BuildAccountController(databaseName);
        ...
        [TestMethod]
        public async Task UserLoginSuccessful()
        {
            //Preparation
            var databaseName = Guid.NewGuid().ToString();
            await CreateUser(databaseName);
            var userInfo = new UserInfo() {EmailAddress = "oluwaseyi@gmail.com", Password = "Pa$$w0rd"};
        ...
        private async Task CreateUser(string databaseName)
        {
            var accountController = BuildAccountController(databaseName);
            var userInfo = new UserInfo() {EmailAddress = "oluwaseyi@gmail.com", Password = "Pa$$w0rd"};
            await accountController.Create(userInfo);
        }

        private AccountController BuildAccountController(string databaseName)
        {
            ...
            {
                {"JWT:key", "KYlNquJdJACUhYmelOs645xqDNadCzUBDSBtbso9"}
            };
